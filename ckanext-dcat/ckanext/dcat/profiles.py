...
format_list ={'API': 'application/api',
'ASCII': 'text/ascii',
...
'WFS': 'text/wfs',
'WMS': 'text/wms',
'WMS-XML': 'application/vnd.ogc.wms_xml',
...
class EuropeanDCATAPProfile(RDFProfile):
    ...
    def graph_from_dataset(self, dataset_dict, dataset_ref):
        ...
        #  Lists
        items = [
            ('language', DC.language, None, Literal),
    ...
    def graph_from_catalog(self, catalog_dict, catalog_ref):
        ...
        items = [
            ...
            ('language', DC.language, config.get('ckan.locale_default', 'en'), Literal),
        ]
        for item in items:
...
class ExtendEuropeanDCATAPProfile(RDFProfile):
    '''
    An RDF profile based on the DCAT-AP for data portals in Europe
    ...
    def graph_from_dataset(self, dataset_dict, dataset_ref):
        ...
        g.add((dataset_ref, DCT.identifier, Literal(config.get('ckan.site_url')+"/dataset/"+dataset_dict['id'])))

        g.add((dataset_ref, DCT.license, URIRef(config.get('ckan.site_license'))))

        for groups in dataset_dict.get('groups', []):
            theme_group = config.get('ckan.site_themeTaxonomy')+groups['name']
            g.add((dataset_ref, DCAT.theme, URIRef(theme_group)))
        ...
        #print ("dataset_dict ******************************************* %s: " % dataset_dict['language'])
        ...
        if any([
            ...
            g.add((contact_details, RDF.type, VCARD.Organization))
            g.add((dataset_ref, DCAT.contactPoint, contact_details))
            g.add((dataset_ref, DC.language, Literal('es')))
        ...
        if any([
            ...
            publisher_details = URIRef(config.get('ckan.site_publisher'))
            g.add((publisher_details, RDF.type, FOAF.Organization))
            g.add((dataset_ref, DCT.publisher, publisher_details))
        ...
        for resource_dict in dataset_dict.get('resources', []):
            ...
            #print ("formato 2 : ************************** %s: " %  resource_dict.get('format', ''))
            #print ("resource_dict **************************** %s " % resource_dict)

            source_format = resource_dict.get('format', '')
            if  source_format.strip()=='':
                source_format = 'HTML'
            try:
                mimeType_final = format_list[source_format]
            except:
                mimeType_final = 'text/html'
                print ("Formato erroneo %s ********************************* " % resource_dict)
                
            g.add((distribution, DCAT.mediaType, Literal(mimeType_final)))
    ...
    def graph_from_catalog(self, catalog_dict, catalog_ref):
        ...
        items = [
            ...
            ('homepage', FOAF.homepage, config.get('ckan.site_url'), URIRef),
            ('language', DC.language, config.get('ckan.locale_default', 'en'), Literal),
            ('issued', DCT.issued, Literal(config.get('ckan.site_publish_date'),datatype=XSD.dateTime), Literal),
            ('publisher', DCT.publisher, config.get('ckan.site_publisher'), URIRef),
            ('themeTaxonomy', DCAT.themeTaxonomy, config.get('ckan.site_themeTaxonomy'), URIRef),
            ('license', DCT.license, config.get('ckan.site_license'), URIRef),
        ]
        for item in items:
            key, predicate, fallback, _type = item
            if catalog_dict:
