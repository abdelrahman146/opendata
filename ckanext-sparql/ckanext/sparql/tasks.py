...
def upload_rdf_data(graph, pkg_data):
    ...
    if pkg_data['storetype'] == 'virtuoso':
        sparqlquery = 'INSERT IN GRAPH <%s> { %s }' % (pkg_data['graph'].encode('utf-8'), triples)
        query = {'query': sparqlquery.encode('utf-8') }
        headers = {'Content-type': 'application/x-www-form-urlencoded'}
        success_codes = [200]

    elif pkg_data['storetype'] == 'sparql11':
        sparqlquery = 'INSERT DATA { GRAPH %s { %s } }' % (pkg_data['graph'].encode('utf-8'), triples)
        query = {'query': sparqlquery.encode('utf-8') }
        headers = {'Content-type': 'application/sparql-update', 'Connection': 'Keep-Alive'}
        success_codes = [200, 204]

    if pkg_data['isauthrequired']:
        r = requests.post(pkg_data['sparulurl'], data=query, headers=headers, auth=HTTPBasicAuth(pkg_data['username'], pkg_data['passwd']))
    else:
        r = requests.post(pkg_data['sparulurl'], data=query, headers=headers)
    print("r.status_code tasks.py: %s" %  r.status_code)
...
def crawl_and_upload_data():
    print("crawl_and_upload_data")
    g = Graph()
    sparql_endpoint = get_global_sparql_enpoint()

    if sparql_endpoint:
        for package in get_package_list():
            request = urllib2.Request(urlparse.urljoin(DATASET_URL, package+".rdf"), headers={"Accept" : "application/rdf+xml"})
