...
class SpatialHarvester(HarvesterBase):
    ...
    def import_stage(self, harvest_object):
        ...
        if status == 'new':
            ...
            try:
                log.info("Entra en crear dataset base.py  context --------------------------------------- " )
                resources =  package_dict['resources']
                for resource in resources:
                    resource_name = resource['name']

                    fileName, fileExtension = os.path.splitext(resource_name)

                    if(fileExtension):
                        fileExtension = fileExtension.lstrip(".")
                        resource['format'] = fileExtension.upper()

                #eliminar este bloque  linea cuando se introduzcan las licencias en geonetwork
                extras = package_dict['extras']
                for extra in extras:
                    if ('licence' == extra['key']):
                        if (extra['value']=='[]'):
                            log.info("licencia vacia, cambiando licencia")
                            extra['value']='cc-by'
                #eliminar esta linea cuando se introduzcan las licencias en geonetwork
                package_dict['license_id'] = 'cc-by'

                #print("create package_dict en base.py de spatial: %s" % package_dict)
            ...
            if not self.force_import and previous_object and harvest_object.metadata_modified_date <= previous_object.metadata_modified_date:
                ...
                try:
                    log.info("Entra en actualizar dataset desde harvest base.py de spatial --------------------------------------- " )
                    resources =  package_dict['resources']
                    for resource in resources:
                        resource_name = resource['name']
                    ...
                    source_dict = logic.get_action('package_show')(context, package_dict)
                    package_dict['groups'] = source_dict['groups']
                    
                    resources_source = source_dict['resources']
                    resources_target = package_dict['resources']
                    for resource in resources_source:
                        if('url_type' in resource and resource['url_type']=='upload'):
                            resources_target.append(resource)
                    ...
                    #log.info('member_of_group: %s',  source_dict)
                    #print("update  package_dict en base.py de spatial: %s" % package_dict)
                    package_id = p.toolkit.get_action('package_update')(context, package_dict)
                    log.info('Updated package w6 %s with guid %s', package_id, harvest_object.guid)
                except p.toolkit.ValidationError, e:
                    self._save_object_error('Validation Error: %s' % str(e.error_summary), harvest_object, 'Import')
                    return False
